...
Generated:       assays (the DIEF-MO identifiers, optionally linked to a sample)
"""
import sqlite3
...
_KEYED_TABLES = {"areas", "matrices", "experiments", "batches", "samples"}
...
def init_db(db_path=DB_PATH):
    with get_conn(db_path) as conn:
        ...
        # Migration: older databases have an 'assays' table without sample_code.
...
def create_assay(experiment, area, matrix, sample_code=None, db_path=DB_PATH):
    ...
    with get_conn(db_path) as conn:
        ...
        conn.execute(
            "INSERT INTO assays(dief_id, experiment_code, area_code, matrix_code, seq, created_at, sample_code) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (dief_id, experiment, area, matrix, seq, _now(), sample_code))
    return dief_id


def create_assay_from_sample(sample_code, area, db_path=DB_PATH):
    """Generate an ID for a registered sample, deriving experiment and matrix
    from the sample's lineage (sample -> batch -> experiment, sample -> matrix)."""
...
def experiment_tree(db_path=DB_PATH):
    """Nested view: experiment -> batches -> samples -> assays."""
    ...
    for e in experiments:
        ...
        for b in [b for b in batches if b["experiment_code"] == e["code"]]:
            ...
            for s in [s for s in samples if s["batch_code"] == b["code"]]:
                ...
                b_node["samples"].append({"sample": s, "assays": s_assays})
...
def counts(db_path=DB_PATH):
    with get_conn(db_path) as conn:
        return {t: conn.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0]
                for t in ("areas", "matrices", "experiments", "batches", "samples", "assays")}
